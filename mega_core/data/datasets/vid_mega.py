...
from .vid import customdata
from mega_core.config import cfg

class VIDMEGADataset(customdata):
    def __init__(self, image_set, data_dir, img_dir, anno_path, img_index, transforms, is_train=True):
        ...
        if not self.is_train:
            ...
            for id, image_index in enumerate(self.image_set_index):
                frame_id = int(image_index.split("_")[-1])
                if frame_id == 0:
    ...
    def _get_train(self, idx):
        ...
        if hasattr(self, "pattern"):
            ...
            for i in range(len(offsets)):
                ...
                ref_filename = self.image_set_index[ref_id] #self.pattern[idx] % ref_id
                img_ref = Image.open(self._img_dir % ref_filename)
            ...
            if cfg.MODEL.VID.MEGA.MEMORY.ENABLE:
                ...
                for i in range(len(offsets)):
                    ...
                    ref_filename = self.image_set_index[ref_id]
                    img_ref = Image.open(self._img_dir % ref_filename)
    ...
    def _get_test(self, idx):
        ...
        if cfg.MODEL.VID.MEGA.GLOBAL.ENABLE:
            ...
            for id in range(size):
                ref_filename= self.pattern[idx] % shuffled_index[(idx - self.start_id[idx] + cfg.MODEL.VID.MEGA.GLOBAL.SIZE - id - 1) % self.frame_seg_len[idx]]
                img = Image.open(self._img_dir % ref_filename)
